<template>
  ...
  <div class="flex h-screen">
    ...
    <!-- Main Content -->
    <div
      class="relative flex flex-row bg-purple-100 ml-1 mt-1 rounded h-full w-full"
    >
      ...
      <div
        class="bg-slate-100 text-black mt-16 ml-1 h-[88vh] w-52 rounded-lg shadow-lg overflow-auto flex-shrink-0 p-1"
      >
        <!-- Title Section -->
        <!-- Title Section -->
        <div class="sticky top-0 z-10 bg-slate-100">
          <button
            class="bg-purple-500 text-white px-4 py-2 rounded-lg w-full font-semibold"
          >
            + Compose
          </button>
        </div>
      ...
      <div
        v-if="selectedUser"
        class="flex-1 flex flex-col bg-white rounded-lg mt-16 ml-1 shadow-lg h-[88vh] overflow-hidden"
      >
        <!-- Chat Header -->
        <div
          class="p-2 bg-purple-500 text-white flex items-center rounded-t-lg"
        >
      ...
      users: [
        {
          name: "Tenma",
          role: "client",
          messages: [{ sender: "client", text: "Hey, how can I help?" }],
        },
        {
          name: "Hoop",
          role: "client",
          messages: [
            { sender: "client", text: "Let's discuss the new design." },
          ],
        },
        {
          name: "Ashton",
          role: "client",
          messages: [
            { sender: "client", text: "I have a question about my order." },
          ],
        },
        {
          name: "Tariq",
          role: "tailor",
          messages: [{ sender: "tailor", text: "The fabric has arrived." }],
        },
        {
          name: "Victor",
          role: "tailor",
          messages: [
            {
              sender: "tailor",
              text: "I need clarification on the measurements.",
            },
          ],
        },
        {
          name: "Chris",
          role: "tailor",
          messages: [{ sender: "tailor", text: "Are we still on schedule?" }],
        },
      ],
      selectedUser: null,
      newMessage: "",
  ...
  computed: {
    filteredUsers() {
      return this.filterType === "all"
        ? this.users
        : this.users.filter((user) => user.role === this.filterType);
    },
  },
  methods: {
    selectUser(user) {
...
.filter-option {
  @apply flex items-center cursor-pointer space-x-2;
}
.indicator {
